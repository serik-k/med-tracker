...
type Filter = 'all' | 'unassigned' | 'en_route' | 'completed';
const orderStore = useOrderStore(); const loading = ref(false); const createError = ref(''); const copiedToken = ref<string | null>(null); const selectedToken = ref<string | null>(null); const isCreateOpen = ref(false); const activeFilter = ref<Filter>('all'); const now = ref(Date.now()); let clockId = 0;
const allOrders = computed(() => orderStore.activeOrders); const activeOrders = computed(() => allOrders.value.filter(o => o.status !== 'COMPLETED')); const selectedOrder = computed(() => allOrders.value.find(o => o.token === selectedToken.value) || null); const isUnassigned = (o: Order) => !o.carNumber?.trim();
...
const filters: { value: Filter; label: string }[] = [{ value: 'all', label: 'Все' }, { value: 'unassigned', label: 'Без бригады' }, { value: 'en_route', label: 'В пути' }, { value: 'completed', label: 'Завершённые' }];
const filteredOrders = computed(() => allOrders.value.filter(o => activeFilter.value === 'all' || (activeFilter.value === 'unassigned' ? isUnassigned(o) && o.status !== 'COMPLETED' : activeFilter.value === 'en_route' ? o.status === 'EN_ROUTE' : o.status === 'COMPLETED')).sort((a,b) => Number(isUnassigned(b))-Number(isUnassigned(a)) || +new Date(a.createdAt)-+new Date(b.createdAt)));
