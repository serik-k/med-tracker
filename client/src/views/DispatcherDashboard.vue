...
const allOrders = computed(() => orderStore.activeOrders); const isCompleted = (o: Order) => o.expired || o.status === 'COMPLETED'; const activeOrders = computed(() => allOrders.value.filter(o => !isCompleted(o))); const selectedOrder = computed(() => activeOrders.value.find(o => o.token === selectedToken.value) || null); const isUnassigned = (o: Order) => !o.carNumber?.trim();
