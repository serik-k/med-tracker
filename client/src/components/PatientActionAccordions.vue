...
const props=defineProps<{order:Order}>(); defineEmits<{(e:'update-access',value:Partial<AccessInfo>):void;(e:'update-symptoms',value:string[]):void}>(); const opened=ref(new Set<string>()); const hasAccess=computed(()=>Object.entries(props.order.accessInfo||{}).some(([key,value])=>key!=='photoUrl'&&Boolean(value))); const items=computed(()=>[{key:'access',title:'Как попасть к пациенту',caption:hasAccess.value?'Данные доступа переданы бригаде':'Домофон, подъезд, этаж и ориентиры',done:hasAccess.value,icon:KeyRound},{key:'symptoms',title:'Симптомы для врача',caption:props.order.symptoms?.length?`Передано симптомов: ${props.order.symptoms.length}`:'Сообщите важные симптомы заранее',done:Boolean(props.order.symptoms?.length),icon:Stethoscope},{key:'ready',title:'Подготовиться к приезду',caption:'Короткая памятка для пациента и близких',done:false,icon:ClipboardCheck}]); const isOpen=(key:string)=>opened.value.has(key); function toggle(key:string){const next=new Set(opened.value);next.has(key)?next.delete(key):next.add(key);opened.value=next;}
</script>
