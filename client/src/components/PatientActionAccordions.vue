<template>
  <section class="space-y-2.5" :aria-label="lang.t('patientPrepAria')">
    <div class="px-1 pb-1"><p class="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">{{ lang.t('actionsNow') }}</p><h2 class="mt-1 text-base font-black">{{ lang.t('helpCrewFaster') }}</h2></div>
    <article v-for="item in items" :key="item.key" class="overflow-hidden rounded-2xl border bg-white shadow-sm transition-colors" :class="isOpen(item.key)?'border-teal-300':'border-slate-200'">
      <button type="button" class="flex min-h-[72px] w-full items-center gap-3 px-4 py-3 text-left" :aria-expanded="isOpen(item.key)" @click="toggle(item.key)">
        ...
        <span v-if="item.done" class="hidden shrink-0 items-center gap-1 rounded-full bg-teal-50 px-2 py-1 text-[10px] font-black text-teal-800 min-[380px]:flex"><CircleCheck class="h-3.5 w-3.5" /> {{ lang.t('done') }}</span>
...
import { computed,ref } from 'vue'; import { ChevronDown,CircleCheck,ClipboardCheck,KeyRound,Stethoscope } from 'lucide-vue-next'; import AccessForm from '@/components/AccessForm.vue'; import SymptomSelector from '@/components/SymptomSelector.vue'; import PreArrivalChecklist from '@/components/PreArrivalChecklist.vue'; import type { AccessInfo,Order } from '@/types'; import { useLangStore } from '@/stores/langStore';
const props=defineProps<{order:Order}>(); defineEmits<{(e:'update-access',value:Partial<AccessInfo>):void;(e:'update-symptoms',value:string[]):void}>(); const lang=useLangStore(); const opened=ref(new Set<string>()); const readyDone=ref(false); const hasAccess=computed(()=>Object.entries(props.order.accessInfo||{}).some(([key,value])=>key!=='residenceType'&&Boolean(value))); const items=computed(()=>[{key:'access',title:lang.t('accessHow'),caption:hasAccess.value?lang.t('accessSentCaption'):lang.t('accessPrompt'),done:hasAccess.value,icon:KeyRound},{key:'symptoms',title:lang.t('symptomsForDoctor'),caption:props.order.symptoms?.length?lang.t('symptomsSentCount').replace('{count}',String(props.order.symptoms.length)):lang.t('symptomsPrompt'),done:Boolean(props.order.symptoms?.length),icon:Stethoscope},{key:'ready',title:lang.t('prepareArrival'),caption:readyDone.value?lang.t('allItemsDone'):lang.t('prepShort'),done:readyDone.value,icon:ClipboardCheck}]); const isOpen=(key:string)=>opened.value.has(key); function toggle(key:string){const next=new Set(opened.value);next.has(key)?next.delete(key):next.add(key);opened.value=next;}
</script>
